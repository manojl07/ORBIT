...
const PostModal = ({ isOpen, onClose, post, }) => {
  ...
  const getPostId = (value) => value?._id ?? value?.id;
  ...
  return (
    <>
      <div className="fixed inset-0 z-150 bg-black/90 flex items-center justify-center p-0 min-[1000px]:p-6" onClick={onClose}>
        <div onClick={(e) => e.stopPropagation()} className="relative w-full h-dvh bg-zinc-950 overflow-hidden flex flex-col min-[1000px]:grid min-[1000px]:grid-cols-[1.6fr_1fr] min-[1000px]:max-w-6xl min-[1000px]:h-[92vh] min-[1000px]:rounded-2xl min-[1000px]:border min-[1000px]border-zinc-800">
          ...
          <div className="w-full h-[40dvh] min-[500px]:h-[42dvh] bg-black flex items-center justify-center overflow-hidden shrink-0 min-[1000px]:h-full min-[1000px]:shrink">
            <img src={currentPost.imageUrl} alt={currentPost.caption || "Post"}
          ...
          <div className="min-h-0 flex-1 bg-zinc-950 flex flex-col overflow-hidden min-[1000px]:h-full">
            ...
            {/* HEADER */}
            <PostHeader post={currentPost} isOwner={isOwner} onClose={onClose} onDeleteClick={() => setShowDeleteDialog(true)} />
            ...
            {currentPost.caption && (
              <div className="px-3 py-2 min-[500px]:px-4 min-[500px]:py-3 min-[1000px]:px-5 min-[1000px]:py-4 border-b border-zinc-800 shrink-0 ">
                <div className="flex gap-2 min-[500px]:gap-3" >
                  <img src={currentPost.user?.profileImg} alt={currentPost.user?.username || ""}
                    className="w-7 h-7 min-[500px]:w-8 min-[500px]:h-8 rounded-full object-cover shrink-0" />
            ...
            {/* COMMENTS */}
            <div className="flex-1 min-h-0 overflow-y-auto instagram-scroll">
            ...
            <div className="px-3 py-2 min-[500px]:px-4 min-[500px]:py-2.5 min-[1000px]:px-5 min-[1000px]:py-3 shrink-0" >
              <p className="text-xs min-[500px]:text-sm font-semibold text-white" >
                ...
                {currentPost.likesCount === 1 ? "like" : "likes"}
              </p>

              <p className="mt-0.5 text-[9px] min-[500px]:text-[10px] uppercase tracking-widertext-zinc-600">
                {new Date(currentPost.createdAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric", })}
              </p>
            </div>
      ...
      {/* DELETE DIALOG */}
      <DeletePostDialog isOpen={showDeleteDialog} post={currentPost}
